...
mod tests {
    ...
    async fn happy_path() {
        ...
        // somewhat arbritrary?
        let refund_timelock = 10;
        let punish_timelock = 30;
        ...
        let lock_tx_fee = bob_btc_wallet.transaction_fee(lock_txid).await.unwrap();
        ...
        assert_eq!(
            ...
            bob_initial_btc_balance - btc_amount - lock_tx_fee
        );
    ...
    async fn bob_refunds() {
        ...
        let alice_initial_xmr_balance = TEN_XMR;
        let bob_initial_xmr_balance = 0;

        monero
            .init(alice_initial_xmr_balance, bob_initial_xmr_balance)
            .await
            .unwrap();
        let alice_monero_wallet = monero::AliceWallet(&monero);
        let _bob_monero_wallet = monero::BobWallet(&monero);
        ...
        // TX_FEE
        let lock_tx_fee = bob_btc_wallet
    ...
    async fn alice_punishes() {
        ...
        // Alice fails to redeem_btc
        // Bob fails refund to refund_btc
        // Alice punishes bob claiming btc and leaving xmr locked?
